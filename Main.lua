...
for _, Module in next,
    game:GetService("ReplicatedStorage"):WaitForChild("src"):WaitForChild("Data"):WaitForChild("Units"):GetDescendants() do
...
function sex()
  ...
  getgenv().amountUnitPlace = data.amountUnitPlace
  ...
  function updatejson()
    ...
    local xdata = {
      ...
      level = getgenv().level,
      door = getgenv().door,
      amountUnitPlace = getgenv().amountUnitPlace,
  ...
  if game.PlaceId == 8304191830 then
    ...
    autofarmtab:Toggle("Auto Start", getgenv().autostart, function(bool)
      ...
      if getgenv().autostart and getgenv().AutoFarm then
        ...
        local owner_room_11 = game:GetService("Workspace")["_LOBBIES"].Story._lobbytemplategreen11.Owner.Value
        local owner_room_10 = game:GetService("Workspace")["_LOBBIES"].Story._lobbytemplategreen10.Owner.Value
        if owner_room_11 == nil then
          getgenv().door = "_lobbytemplategreen11"
        elseif owner_room_10 == nil then
          getgenv().door = "_lobbytemplategreen10"
        elseif owner_room_11 ~= nil and owner_room_10 ~= nil then
    ...
    autofarmtab:Dropdown("Select Amount to Place", { 1, 2, 3, 4, 5, 6 }, getgenv().amountUnitPlace, function(amount)
      getgenv().amountUnitPlace = amount
      updatejson()
    end)
    ...
    getgenv().diff = autofarmtab:Dropdown("Select Difficulty", { "Normal", "Hard" }, getgenv().difficulty,
      function(diff)
    ...
    autofarmtab:Label("Amount to Place: " .. tostring(getgenv().amountUnitPlace))
  ...
  if game.PlaceId == 8304191830 then
    ...
    getgenv().autoOpenStarOcean = false
    ...
    misc:Toggle("Auto Open Star Ocean Egg", getgenv().autoOpenStarOcean, function(bool)
      getgenv().autoOpenStarOcean = bool
      while getgenv().autoOpenStarOcean do
        task.wait(0.3)
        local args = {
          [1] = "capsule_marineford"
        }
  ...
  while task.wait() do
    ...
    if getgenv().AutoFarm and not getgenv().disableatuofarm then
      if game.PlaceId ~= 8304191830 then
        ...
        for i = 1, getgenv().amountUnitPlace do
          local unitinfo = getgenv().SelectedUnits["U" .. i]
          if unitinfo ~= nil then
            local unitinfo_ = unitinfo:split(" #")
